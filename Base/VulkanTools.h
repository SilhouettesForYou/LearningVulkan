...
namespace vks
{
    namespace tools
    {
        ...
        std::string PhysicalDeviceTypeString(VkPyhsicalDeviceType type);

        // Selected a suitable supported depth format starting with 32 bit down to 16 bit
		// Returns false if none of the depth formats in the list is supported by the device
        VkBool32 GetSupportDepthFormat(VkPhysicalDevice physicalDevice, VkFormat* depthFormat);
...
#else
        VkShaderModlue LoadShader(const char* filename, VkDevice device);
#endif
