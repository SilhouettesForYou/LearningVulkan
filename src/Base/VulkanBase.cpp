...
void VulkanBase::UpdateOverlay()
{
	...
	if (UIOverlay.Update() || UIOverlay.update) 
    {
		BuildCommandBuffers();
		UIOverlay.update = false;
	}
...
void VulkanBase::PrepareFrame()
{
    ...
    // Recreate the swap chain if it's no longer compatible with the surface (OUT_OF_DATE) or no longer optimal for presnetation (SUBOPTIMAL)
...
bool VulkanBase::InitVulkan()
{
	...
	VkBool32 validDepthFormat = vks::tools::GetSupportDepthFormat(physicalDevice, &depthFormat);
