...
class VulkanExampleRuntimeMipMapGeneration : public VulkanBase
{
public:
	struct Texture
	{
	...
	VulkanExampleRuntimeMipMapGeneration() : VulkanBase(ENABLE_VALIDATION)
	{
	...
	~VulkanExampleRuntimeMipMapGeneration()
	{
	...
	void LoadTexture(std::string filename, VkFormat format, bool forceLinearTiling)
	{
		...
		// Calculated as log2(max(width, height, depth))c + 1
		...
		memAllocInfo.memoryTypeIndex = vulkanDevice->GetMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device, &memAllocInfo, nullptr, &stagingMemory));
		VK_CHECK_RESULT(vkBindBufferMemory(device, stagingBuffer, stagingMemory, 0));

		// Copy texture data into stading buffer
		uint8_t* data;
...
VULKAN_EXAMPLE_MAIN(VulkanExampleRuntimeMipMapGeneration)